...
export async function fetchDevPoolKeys(
  connection: Connection,
  poolId: PublicKey,
  version: number = 4
) {
...
export async function getRouteRelated(
  ...
  const candidateTokenMints = routeMiddleMints.concat([
    tokenInMintString,
    tokenOutMintString,
  ]);
  ...
  const routeRelated = allPoolKeys.filter((info) => {
    const isCandidate =
      candidateTokenMints.includes(info.baseMint.toBase58()) &&
      candidateTokenMints.includes(info.quoteMint.toBase58());
    const onlyInRoute =
      onlyRouteMints.includes(info.baseMint.toBase58()) &&
      onlyRouteMints.includes(info.quoteMint.toBase58());
